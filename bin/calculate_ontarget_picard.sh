...
# Uso:
#   calculate_ontarget_picard.sh <cram> <fastq_r1_ignored> <outdir> <sample> <reference_fasta> [cram_stats_opt]
CRAM="$1"
R1_IGNORED="$2"
OUTDIR="$3"
SAMPLE="$4"
...
[[ -n "${CRAM_STATS}" ]] && echo "[INFO] cram_stats=${CRAM_STATS}" >> "$log"

# ---------- Denominador: raw total sequences ----------
raw_total="NA"
if [[ -n "${CRAM_STATS}" && -s "${CRAM_STATS}" ]]; then
  raw_total=$(awk -F':' '/^raw total sequences:/ {gsub(/^[ \t]+/,"",$2); print $2; exit}' "${CRAM_STATS}" 2>/dev/null || echo "NA")
fi
if [[ "${raw_total}" == "NA" || -z "${raw_total}" ]]; then
  # fallback (no debería hacer falta si pasas CRAM_STATS)
  raw_total=$(samtools stats -T "${REF_FASTA}" "${CRAM}" 2>/dev/null | awk -F'\t|: ' '/^SN\traw total sequences:/ {print $NF; exit}' || echo "NA")
fi
...
if [[ ! -s "${SAMPLE}.picard.aln_metrics.txt" ]]; then
  ...
  echo -e "%UsableReads\tNA" > "${SAMPLE}.ontarget_result.txt"
  exit 0
fi

# ---------- Localizar índice PF_READS_ALIGNED en cabecera ----------
...
if [[ -z "${col_pf_aligned}" ]]; then
  echo "[ERROR] PF_READS_ALIGNED no está en la cabecera de Picard" >> "$log"
  echo -e "%UsableReads\tNA" > "${SAMPLE}.ontarget_result.txt"
  exit 0
fi
...
echo -e "%UsableReads\t${pct}" > "${SAMPLE}.ontarget_result.txt"
